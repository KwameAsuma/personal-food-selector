  ...
  return (
    <div id="menu-grid-container" className="py-2">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-xl font-bold tracking-wide text-zinc-100 font-brand flex items-center gap-2">
          <span>Popular Dishes</span>
          <span className="text-[11px] bg-emerald-500/20 border border-emerald-500/30 text-emerald-400 font-bold font-sans px-2.5 py-0.5 rounded-full uppercase">
            {items.length} choices
          </span>
        </h2>
        <div className="text-[10px] text-zinc-300 font-bold uppercase tracking-wider hidden sm:block font-sans">
          ...
          return (
            ...
            >
              <div>
                ...
                <div className="w-full aspect-[4/3] bg-zinc-800/50 rounded-2xl mb-4.5 overflow-hidden relative shadow-sm border border-white/5">
                  {item.imageUrl ? (
                    <img 
                      src={item.imageUrl} 
                      alt={item.name}
                  ...
                  <div className="absolute top-3 right-3 w-8 h-8 rounded-full bg-black/60 backdrop-blur-md shadow-sm flex items-center justify-center text-base font-bold select-none border border-white/10 z-10 text-white">
                    {item.emoji}
                  </div>
                </div>
                ...
                <div className="flex items-start justify-between gap-2">
                  <h3 className="font-bold text-lg text-zinc-100 group-hover:text-emerald-400 transition-colors tracking-wide font-brand">
                    {item.name}
                  </h3>
                </div>

                <p className="text-zinc-200 text-[11px] mt-1.5 leading-relaxed font-medium">
                  {item.description}
                </p>
              </div>
              ...
              <div className="mt-5 pt-3.5 border-t border-white/5 flex items-center justify-between">
                <div className="flex items-center gap-1.5 text-xs font-bold text-emerald-400 font-mono">
                  {((item.name.length % 5) * 0.1 + 4.5).toFixed(1)} <Star className="w-3.5 h-3.5 fill-emerald-400" />
                </div>
                <span className="text-[10px] text-zinc-300 group-hover:text-emerald-400/80 font-bold uppercase tracking-wider flex items-center gap-1.5 font-mono transition-colors">
                  <Settings className="w-3.5 h-3.5 text-zinc-300 group-hover:text-emerald-400 group-hover:rotate-90 transition-all duration-500 ease-out" />
                  <span>Customize</span>
                </span>
                <span className="w-7 h-7 rounded-lg bg-white/5 group-hover:bg-emerald-500 text-zinc-200 group-hover:text-white flex items-center justify-center transition-all duration-300 shadow-sm shrink-0 border border-white/5 group-hover:border-emerald-400">
