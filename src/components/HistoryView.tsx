    ...
    >
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-zinc-400 mt-2 font-medium">Your previously saved plates from the cloud.</p>
        </div>
        <button
      ...
      {isLoading ? (
        ...
        <div className="text-center py-20 bg-zinc-900/50 rounded-[32px] border border-white/5">
          <p className="text-zinc-400 text-lg">You haven't saved any plates yet.</p>
        </div>
      ) : (
        <div className="grid gap-6">
          {history.map((record) => (
            <div key={record.id} className="bg-zinc-900 border border-white/10 rounded-3xl p-6 shadow-xl relative overflow-hidden">
              <button 
              ...
              <div className="mb-4">
                <h3 className="text-xl font-bold text-white font-brand">{record.name}</h3>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {record.meals.map((meal: CustomizedMeal, i: number) => (
                  <div key={i} className="bg-zinc-800/50 rounded-xl p-3 border border-white/5 flex items-center gap-3">
                    <div className="text-3xl">{meal.baseItem.emoji}</div>
                    <div className="flex-1">
                      <div className="font-bold text-sm text-white leading-tight">{meal.baseItem.name}</div>
